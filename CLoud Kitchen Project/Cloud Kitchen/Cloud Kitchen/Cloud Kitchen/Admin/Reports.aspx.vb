        ...
        If gvReport.Rows.Count > 0 Then
            Select Case reportType
                ...
                Case "inventory_stock"
                    gvReport.HeaderRow.Cells(0).Text = "Ingredient ID"
                    gvReport.HeaderRow.Cells(1).Text = "Ingredient Name"
                    gvReport.HeaderRow.Cells(2).Text = "Stock Quantity"
                    gvReport.HeaderRow.Cells(3).Text = "Unit"
                    gvReport.HeaderRow.Cells(4).Text = "Cost / Unit"
                    gvReport.HeaderRow.Cells(5).Text = "Low Stock Threshold"
                    gvReport.HeaderRow.Cells(6).Text = "Last Updated"
                    FormatCurrencyColumn(4)
                    FormatDateColumn(6)

                Case "inventory_usage"
                    gvReport.HeaderRow.Cells(0).Text = "Ingredient Name"
                    gvReport.HeaderRow.Cells(1).Text = "Unit"
                    gvReport.HeaderRow.Cells(2).Text = "Total Consumed"

                Case "recipe_costing"
                    gvReport.HeaderRow.Cells(0).Text = "Dish Name"
                    gvReport.HeaderRow.Cells(1).Text = "Selling Price"
                    gvReport.HeaderRow.Cells(2).Text = "Est. Ingredient Cost"
                    gvReport.HeaderRow.Cells(3).Text = "Gross Profit Margin"
                    FormatCurrencyColumn(1)
                    FormatCurrencyColumn(2)
                    FormatCurrencyColumn(3)
    ...
    Private Function GenerateReport(ByVal reportType As String, ByVal startDate As String, ByVal endDate As String) As DataTable
        Using conn As New SqlConnection(connString)
            ...
            Select Case reportType
                ...
                Case "inventory_stock"
                    query = "SELECT ingredient_id, ingredient_name, stock_quantity, unit, cost_per_unit, low_stock_threshold, last_updated FROM Ingredients ORDER BY ingredient_name ASC"

                Case "inventory_usage"
                    query = "SELECT I.ingredient_name, I.unit, SUM(DI.qty_required * OD.quantity) AS total_consumed " &
                            "FROM Order_Details OD " &
                            "INNER JOIN Orders O ON OD.order_id = O.order_id " &
                            "INNER JOIN Dish_Ingredients DI ON OD.m_id = DI.m_id " &
                            "INNER JOIN Ingredients I ON DI.ingredient_id = I.ingredient_id " &
                            "WHERE O.order_status = 'Completed' "
                    If Not String.IsNullOrEmpty(startDate) AndAlso Not String.IsNullOrEmpty(endDate) Then
                        query &= " AND O.order_date BETWEEN @StartDate AND @EndDate "
                    End If
                    query &= "GROUP BY I.ingredient_name, I.unit ORDER BY total_consumed DESC"

                Case "recipe_costing"
                    query = "SELECT M.m_name, M.m_final_price AS selling_price, " &
                            "ISNULL(SUM(DI.qty_required * I.cost_per_unit), 0) AS est_ingredient_cost, " &
                            "(M.m_final_price - ISNULL(SUM(DI.qty_required * I.cost_per_unit), 0)) AS gross_profit " &
                            "FROM menu_item M " &
