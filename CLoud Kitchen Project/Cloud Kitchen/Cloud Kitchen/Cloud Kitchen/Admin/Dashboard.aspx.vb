    ...
    Private Sub LoadDashboardData()
        Try
            Using conn As New SqlConnection(ConfigurationManager.ConnectionStrings("constr").ConnectionString)
                ...
                Dim lowStockQuery As String = "SELECT COUNT(*) FROM Ingredients WHERE stock_quantity <= low_stock_threshold"
                Dim lowStockCountObj As Object = New SqlCommand(lowStockQuery, conn).ExecuteScalar()
                Dim lowStockCount As Integer = If(IsDBNull(lowStockCountObj) OrElse lowStockCountObj Is Nothing, 0, Convert.ToInt32(lowStockCountObj))
                lblLowStockAlerts.Text = lowStockCount.ToString()

                Dim outStockQuery As String = "SELECT COUNT(*) FROM Ingredients WHERE stock_quantity <= 0"
                Dim outStockCountObj As Object = New SqlCommand(outStockQuery, conn).ExecuteScalar()
                Dim outStockCount As Integer = If(IsDBNull(outStockCountObj) OrElse outStockCountObj Is Nothing, 0, Convert.ToInt32(outStockCountObj))

                If outStockCount > 0 Then
                    pnlLowStockBanner.Visible = True
                    lblBannerLowStockCount.Text = lowStockCount.ToString()
                    lnkRestockBanner.NavigateUrl = "ManageInventory.aspx?filter=OutOfStock"
                    lnkRestockBanner.Text = "🔴 Renew Out of Stock Items (" & outStockCount & ") ➔"
                ElseIf lowStockCount > 0 Then
                    pnlLowStockBanner.Visible = True
                    lblBannerLowStockCount.Text = lowStockCount.ToString()
                    lnkRestockBanner.NavigateUrl = "ManageInventory.aspx?filter=LowStock"
                    lnkRestockBanner.Text = "🟡 Restock Low Stock Alerts (" & lowStockCount & ") ➔"
                Else
                    pnlLowStockBanner.Visible = False
                End If
            End Using
        Catch ex As Exception
