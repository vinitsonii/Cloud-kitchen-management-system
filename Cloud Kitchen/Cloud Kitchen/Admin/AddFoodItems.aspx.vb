...
Public Class AddFoodItems
    ...
    Private Sub BindCategories3()
        Using conn As New SqlConnection(connString)
            Dim cmd As New SqlCommand("SELECT * FROM menu_category where category_status=1", conn)
            conn.Open()
    ...
    Private Sub BindCuisines3()
        Using conn As New SqlConnection(connString)
            Dim cmd As New SqlCommand("SELECT * FROM cuisine_type where cuisine_status=1", conn)
            conn.Open()
            Dim rdr As SqlDataReader = cmd.ExecuteReader()
        ...
        Try
            ...
            Using conn As New SqlConnection(connString)
                Using cmd As New SqlCommand(query, conn)
                    ...
                    conn.Open()
                    cmd.ExecuteNonQuery()
                    lblmsg3.Text = "Menu Item Added Successfully :)"
                    lblmsg3.ForeColor = Drawing.Color.DarkGreen
                End Using
            End Using
    ...
    Private Sub UpdateMenuItem(ByVal name As String, ByVal categoryId As String, ByVal cuisineId As String, ByVal description As String, ByVal price As Decimal, ByVal discount As Decimal, ByVal finalPrice As Decimal, ByVal imageUrl As String, ByVal availability As String, ByVal featured As String, ByVal status As String)
        Using conn As New SqlConnection(connString)
            ...
            conn.Open()
            cmd.ExecuteNonQuery()
            lblmsg3.Text = "Menu Item Updated Successfully :)"
            lblmsg3.ForeColor = Drawing.Color.DarkGreen
        ...
        If e.CommandName = "EditItem" Then
            ...
            Using conn As New SqlConnection(connString)
                ...
                If rdr.Read() Then
                    ...
                    fn.Value = rdr("m_image_url").ToString
                End If
            End Using
    ...
    Protected Sub txtPrice3_TextChanged(ByVal sender As Object, ByVal e As EventArgs)
        CalculateFinalPrice()
    End Sub

    Protected Sub txtDiscount3_TextChanged(ByVal sender As Object, ByVal e As EventArgs)
        CalculateFinalPrice()
    End Sub

    Private Sub CalculateFinalPrice()
        Dim price As Decimal = 0
        Dim discount As Decimal = 0

        If Decimal.TryParse(txtPrice3.Text, price) AndAlso Decimal.TryParse(txtDiscount3.Text, discount) Then
            Dim finalPrice As Decimal = price - (price * discount / 100)
            txtFinalPrice3.Text = finalPrice.ToString()
        End If
    End Sub
